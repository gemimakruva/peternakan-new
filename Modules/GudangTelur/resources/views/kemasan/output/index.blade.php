...
<x-page-header title="List Output Kemasan" :breadcrumbs="['Kemasan' => route('gudang-telur.kemasan-inventory.index'), 'Output' => '']">
    <x-slot name="actions">
...
<div class="mx-1000">
    ...
    <x-filter-panel action="{{ route('gudang-telur.kemasan-output.index', request()->all()) }}">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    ...
                                    <form
                                        action="{{ route('gudang-telur.kemasan-output.destroy', $data->id) }}"
                                        method="post"
                                        class="form-delete"
    ...
    <div class="mobile-card-list d-md-none">
        @forelse($datas as $data)
            <x-mobile-card title="{{ $data->tanggal->translatedFormat('l, d F Y') }}" subtitle="{{ $data->nama_pic_user }}">
                <x-slot name="actions">
                    <a href="{{ route('gudang-telur.kemasan-output.edit', $data->id) }}" class="btn btn-warning btn-sm text-white">
                        <i class="fas fa-edit"></i> Edit
                    </a>
                    <form action="{{ route('gudang-telur.kemasan-output.destroy', $data->id) }}" method="post" class="form-delete" data-tanggal="{{ $data->tanggal->translatedFormat('l, d F Y') }}">
                        @csrf
                        @method('delete')
        ...
        @empty
            <x-empty-state icon="box" title="Belum Ada Data" description="Data output kemasan tidak tersedia." />
        @endforelse
        @if ($datas->hasPages())
...
<script>
    $(document).on('submit', '.form-delete', function (e) {
        ...
        Swal.fire({
            title: `Hapus Data Output Tanggal "${tanggal}"?`,
