...
@section('content_header')
    <x-page-header title="Mixing" :breadcrumbs="['Mixing' => '']">
        <x-slot name="actions">
...
<div class="mx-1200">
    ...
    <x-filter-panel action="{{ route('gudang-pakan.pakan-mixing.index') }}" resetUrl="{{ route('gudang-pakan.pakan-mixing.index') }}">
    ...
    <div class="mobile-card-list d-md-none">
        @forelse($datas as $data)
            <x-mobile-card title="{{ $data->nama_formulasi }}" subtitle="{{ $data->tanggal->translatedFormat('l, d F Y - H:i') }}">
                ...
                <x-slot name="actions">
                    <a href="{{ route('gudang-pakan.pakan-mixing.edit', $data->id) }}" class="btn btn-warning btn-sm text-white">
                        <i class="fas fa-edit"></i> Edit
                    </a>
                    <form action="{{ route('gudang-pakan.pakan-mixing.destroy', $data->id) }}" method="post" class="form-delete flex-1" data-nama_formulasi="{{ $data->nama_formulasi }}">
                        @csrf
                        @method('delete')
