...
<div class="card mb-4">
    ...
    <select id="kandang_id" name="kandang_id" class="form-control" required>
        <option value="" disabled {{ isset($ovkPakan) ? '' : 'selected' }}>Pilih Kandang</option>
       
        @foreach ($kandang as $item)
            <option value="{{ $item->id }}"
                {{ isset($ovkPakan) && $ovkPakan->kandang_id == $item->id ? 'selected' : '' }}>
                {{ $item->nama }}
            </option>
        @endforeach
    </select>
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <select name="waktu_pemberian_pakan" id="waktu_pemberian" class="form-control">
            ...
            <option value="pagi" {{ old('waktu_pemberian_pakan', $ovkPakan->waktu_pemberian_pakan ?? '') == 'pagi' ? 'selected' : '' }}>Pagi</option>
            <option value="sore" {{ old('waktu_pemberian_pakan', $ovkPakan->waktu_pemberian_pakan ?? '') == 'sore' ? 'selected' : '' }}>Sore</option>
            <option value="pagi_sore" {{ old('waktu_pemberian_pakan', $ovkPakan->waktu_pemberian_pakan ?? '') == 'pagi_sore' ? 'selected' : '' }}>Pagi & Sore</option>
        </select>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <div class="mb-3" id="section-pagi">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <input type="number" id="proporsi_pagi" name="proporsi_pemberian_pagi" class="form-control"
                        value="{{ old('proporsi_pemberian_pagi', $ovkPakan->proposi_pemberian_pagi ?? 60) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Perhitungan Pakan Pagi (kg)</label>
                    <input type="number" step="0.01" id="pakan_pagi" name="perhitungan_kebutuhan_pakan_pagi" class="form-control" readonly
                        value="{{ old('perhitungan_kebutuhan_pakan_pagi', $ovkPakan->perhitungan_kebutuhan_pakan_pagi ?? '') }}">
                </div>
            </div>
        </div>
        ...
        <div class="mb-3" id="section-sore">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <input type="number" id="proporsi_sore" name="proporsi_pemberian_sore" class="form-control"
                        value="{{ old('proporsi_pemberian_sore', $ovkPakan->proposi_pemberian_sore ?? 40) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Perhitungan Pakan Sore (kg)</label>
                    <input type="number" step="0.01" id="perhitungan_kebutuhan_pakan_sore" name="perhitungan_kebutuhan_pakan_sore" class="form-control" readonly
                        value="{{ old('perhitungan_kebutuhan_pakan_sore', $ovkPakan->perhitungan_kebutuhan_pakan_sore ?? '') }}">
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <input type="number" step="0.01" id="total_ovk" name="perhitungan_kebutuhan_ovk" class="form-control" readonly
                    value="{{ old('perhitungan_kebutuhan_ovk', $ovkPakan->perhitungan_kebutuhan_ovk ?? '') }}">
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function () {
    function toggleWaktu() {
        ...
        if (waktu === 'pagi') $('#section-pagi').show();
        else if (waktu === 'sore') $('#section-sore').show();
    ...
    function hitungSemua() {
        ...
        if (waktu === 'pagi') pakanPagi = totalPakan * propPagi / 100;
        else if (waktu === 'sore') pakanSore = totalPakan * propSore / 100;
        else if (waktu === 'pagi_sore') {
        ...
        $('#perhitungan_kebutuhan_pakan_sore').val(pakanSore.toFixed(2));
        let totalOVK = totalPakan * dosisOVK / 1000;
        $('#total_ovk').val(totalOVK.toFixed(2));
    }

    $('#waktu_pemberian').on('change', toggleWaktu);
    $('#total_pakan, #dosis_ovk, #proporsi_pagi, #proporsi_sore').on('keyup change', hitungSemua);
    toggleWaktu();
        ...
        if (kandangId) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // Tambahkan default dari DB saat edit
                    if(selectedFlock) {
    ...
    $('#kandang_id').change(function() {
        ...
        loadFlocks(kandangId);
    });

    // Load default saat edit
    @if(isset($ovkPakan))
        var oldKandang = "{{ $ovkPakan->kandang_id }}";
        var oldFlock = "{{ $ovkPakan->flock_id }}";

        if(oldKandang) {
            $('#kandang_id').val(oldKandang);
            loadFlocks(oldKandang, oldFlock);
        }
    @endif
});
</script>
