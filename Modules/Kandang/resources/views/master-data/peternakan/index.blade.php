...
<div class="mb-4 text-center d-flex flex-column align-items-center">
    <h2 class="h4 fw-bold text-dark">Manajemen Peternakan</h2>
    <span class="text-muted mb-0" style="max-width: 600px;">
        Halaman ini digunakan untuk mengelola data Peternakan, termasuk penambahan, pembaruan, dan penghapusan data.
    </span>
</div>
        ...
        <div class="card-header text-white d-flex justify-content-between align-items-center"
            ...
            <form action="{{ route('master-data.peternakan.index', request()->all()) }}" method="get" class="w-100">
                <div class="d-flex justify-content-between align-items-center">
                    <h2 class="card-title mb-0 text-center">Daftar Peternakan</h2>

                    <div class="d-flex" style="gap: .5em">
                        <input type="search" 
                               name="search" 
                               class="form-control form-control-sm" 
                               placeholder="Carui Peternakan..." 
                        ...
                        @can('Tambah Flock')
                        <a href="{{ route('master-data.peternakan.create') }}" class="btn btn-light btn-sm text-dark" title="Tambah Kandang">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <div class="d-flex justify-content-center" style="gap: .5em">
                            @can('Edit Peternakan')
                            <a href="{{ route('master-data.peternakan.edit', $row->id) }}" 
                            ...
                            @can('Hapus Peternakan')
                            <form action="{{ route('master-data.peternakan.destroy', $row->id) }}" 
                                  method="post" 
                                  data-nama="{{ $row->nama }}" 
                                  class="form-delete">
                                @csrf
