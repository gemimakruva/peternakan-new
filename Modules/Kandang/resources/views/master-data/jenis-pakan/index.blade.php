...
@section('content_header')
    <x-page-header title="Jenis Pakan" :breadcrumbs="['Master Data' => '#', 'Jenis Pakan' => '']">
        <x-slot name="actions">
...
@section('content')
    <div class="mx-1200">
        ...
        <x-filter-panel action="{{ route('master-data.jenis-pakan.index', request()->all()) }}" resetUrl="{{ route('master-data.jenis-pakan.index') }}">
        ...
        <div class="mobile-card-list d-md-none">
            @forelse($jenisPakan as $row)
                <x-mobile-card title="{{ $row->nama }}">
                    <x-slot name="actions">
                        <a href="{{ route('master-data.jenis-pakan.edit', $row) }}" class="btn btn-warning btn-sm text-white">
            ...
            @empty
                <x-empty-state icon="clipboard" title="Belum Ada Data" description="Tidak ada data jenis pakan ditemukan." />
            @endforelse
